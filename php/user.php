...
class User{
    ...
    */
    ...
    function loginUser($email, $password){
        ...
        {
            return ['errorMessage' =>'<div style=" color:#fff; padding:10px; width: 100%; background-color:red; border-radius:10px;">
           <small> email and password are required</small>
       </div>'];
                ...
                try{
                    ...
                    if($find_email->rowCount() === 1){
                        ...
                        if($match_pass){
                            $role =$this->db->prepare("SELECT * FROM `users` WHERE Role = ? ");
                            $role->execute([$this->Role]);
                                if($role->rowCount() ===1){
                                    $_SESSION = [
                                        'user_id' => $row['id'],
                            ...
                            return ['errorMessage' => '<div  style="color:#fff; padding:10px; width: 100%; background-color:red; border-radius:10px;">
                            <small> wrong password</small></div>'];
                        }
